...
If your storage only supports `ReadWriteOnce` (RWO), you must restrict all Talismã pods to run on a single node (using `nodeSelector` or `podAffinity`) but that is an antipattern in Kubernetes.
...
```bash
helm install talisma ./charts/talisma
```
...
```yaml
global:
  existingSecret: "my-talisma-secrets"
```
...
To uninstall/delete the `talisma` deployment:

```bash
helm uninstall talisma
```
