"""Talismã's built-in MCP (Model Context Protocol) server.

Runs as a separate container (`mcp-server` in docker-compose, gated by the
`agents` profile). Exposes Talismã's read APIs as MCP tools over JSON-RPC 2.0.
...
then triggers the existing Talismã write endpoint. This keeps MCP safe.
"""
